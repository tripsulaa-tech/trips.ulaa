...
// =============================================================================
// The invoice, as a single self-contained HTML document string.
//
// Kept in its own file (separate from the PDF-rendering pipeline in
// src/utils/invoicePdf.ts) for two reasons:
//   1. It's plain, previewable HTML — copy buildInvoiceHtml()'s output into
//      a .html file and open it directly in a browser to see exactly what
//      will be rasterized, without going through the PDF export flow.
//   2. It keeps "what the invoice looks like" separate from "how we turn
//      that into a PDF blob", so either can change without touching the
//      other.
//
// Layout notes for anyone editing this template — html2canvas (used to
// rasterize this HTML for the PDF, see invoicePdf.ts) has two specific,
// known weak spots that this template deliberately avoids:
//   - flexbox `gap` is unreliable, especially on flex containers nested
//     inside other flex containers (icon+text rows are the classic case:
//     the icon and its text can render as if they were in separate
//     columns, at different vertical spacing). Spacing here uses margins
//     on the child elements instead of `gap` on the container.
//   - `border-radius` + `overflow: hidden` does not reliably clip a
//     <table> element in ANY browser (not just html2canvas — it's a real
//     CSS limitation, since tables lay out their own box model). Where a
//     table needs rounded/clipped corners, the radius + clip go on a
//     wrapping <div> instead, never on the <table> itself.
// =============================================================================
...
// detached render frame on the same origin.
// ---------------------------------------------------------------------------
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .contact-list { display: flex; flex-direction: column; margin-top: 7px; }

  /* Icon + text rows: no flex gap here on purpose (see file header note) —
     the icon gets its own right margin instead, and each row gets its own
     bottom margin, so spacing can't drift between the icon column and the
     text column when this gets rasterized. */
  .contact-row {
  ...
  .booking-id-badge {
    ...
    background: #a0522d; color: #fff; white-space: nowrap;
    font-size: 11.5px; font-weight: 700; line-height: 25px;
    padding: 0 15.7px 0 16px; height: 25px;
  ...
  .invoice-date-row span { line-height: 1; }

  .cal-icon {
    width: 18px; height: 18px; border-radius: 4px;
    background: #ecdece; display: flex; align-items: center;
    justify-content: center; color: #a0522d; margin-right: 6px;
  }
  ...
  /* ── TWO-COLUMN DETAILS ─────────────────────────── */
  /* display:grid deliberately avoided here — html2canvas (see file header
     note) has poor, inconsistent support for CSS Grid: column widths and
     row heights can come out wrong, or collapse into a single stacked
     column, depending on scale factor and content. Flexbox with explicit
     widths (no gap property, per the same note) reproduces the exact same
     two-column, evenly-split layout without that risk. */
  .details-grid {
    display: flex; margin-bottom: 26px;
  }
  .details-grid > div { width: 50%; }
  .details-grid > div:first-child { padding-right: 14px; }
  .details-grid > div:last-child { padding-left: 14px; }

  .section-pill {
    display: inline-block;
    background: #a0522d; color: #fff; white-space: nowrap;
    font-size: 9.5px; font-weight: 700; letter-spacing: 0.9px;
    text-transform: uppercase; line-height: 23px; height: 23px;
    padding: 0 15.1px 0 16px; border-radius: 4px;
  ...
  /* ── PRICE SUMMARY ──────────────────────────────── */
  /* Same reasoning as .details-grid above — flex with equal flex-basis
     replaces CSS Grid so html2canvas renders three genuinely equal-width
     columns instead of risking a collapsed/stacked or unevenly-sized
     result. */
  .price-summary {
    display: flex;
  ...
  .price-amount { font-size: 19px; font-weight: 800; line-height: 1; }
  ...
  /* Rounded/clipped corners live on this wrapper, not on the <table> — a
     <table> ignores border-radius + overflow:hidden in every browser
     (its own box model doesn't clip that way), so putting the radius here
     is the only way the header row's corners actually render rounded. */
  ...
  .payment-table th.status-col { padding-right: 13.1px; }
  ...
  .status-badge {
    ...
    letter-spacing: 0.4px; text-transform: uppercase; line-height: 16px;
    padding: 0 8.6px 0 9px; border-radius: 999px; white-space: nowrap;
  }
  ...
  .follow-label {
    ...
    color: rgba(255,255,255,0.85); margin-right: 12px;
  }
