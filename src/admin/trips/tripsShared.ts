...
/** Shared TripForm shape, empty-state constants, and small pure helpers used
 *  by every piece of the AdminTrips split (useTripsData, useTripForm,
 *  useTripImportExport, AdminTripFormModal, AdminTripDetailModal).
 *
...
export const STORAGE_BUCKET = 'ulaa';
export const isStorageUrl = (url: string) => url.includes(`/object/public/${STORAGE_BUCKET}/`);

// Collects every image URL currently in a TripForm into a flat Set. Used by
// closeModal (useTripForm) to detect/clean up orphaned uploads.
export const collectTripFormUrls = (f: TripForm): Set<string> => {
