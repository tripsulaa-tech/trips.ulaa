...
import { type TripForm, emptyForm, emptyFounder, emptyEndBanner, computeDuration, collectTripFormUrls } from './tripsShared';

/** Owns the "Export Template" / "Import Template" JSON round-trip on the Add
 *  Trip form — building a blank annotated template for an external tool
 *  (e.g. ChatGPT, given trip photos) to fill in, and parsing a filled-in
 *  template back into a TripForm the admin can review before saving.
 *
 *  Needs write-access to the Add/Edit form's own state (useTripForm), so it
 *  takes that hook's setters as arguments rather than owning them itself.
 *
 *  Extracted from AdminTrips.tsx (see that file's git history for the
 *  original single-component version). */
export function useTripImportExport(
  setEditingTrip: (trip: UpcomingTrip | null) => void,
  setForm: (form: TripForm) => void,
  initialModalUrlsRef: React.MutableRefObject<Set<string>>,
  ...
  return { importInputRef, handleExportTemplate, handleImportFile, handleImportInputChange };
}
