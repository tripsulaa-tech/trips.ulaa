...
import type { UpcomingTrip, FounderContent } from '../../types/types-index';
...
import {
  type TripForm, emptyForm, emptyFounder, emptyEndBanner, computeDuration, collectTripFormUrls,
  STORAGE_BUCKET, isStorageUrl,
} from './tripsShared';

/** Owns the Add/Edit Trip form itself — the form state, opening it for
 *  create/edit (including pre-filling the founder block and orphan-upload
 *  tracking), closing it (which sweeps any images uploaded-but-never-saved
 *  this session), and saving. useTripImportExport hooks into this via the
 *  setters it's given. The trips list/table's own state lives in
 *  useTripsData.
 *
 *  Extracted from AdminTrips.tsx (see that file's git history for the
 *  original single-component version). */
export function useTripForm(load: () => void) {
  ...
  const openCreate = async () => {
    ...
    let preFilledFounder = emptyFounder;
    try {
  ...
  return {
    ...
    form, setForm,
    initialModalUrlsRef,
    closeModal, openCreate, openEdit, handleSave, commitGroupBulletDraft,
  };
}
